...
require(grpreg) # group lasso regression
...
function(input, output, session) {
  ...
  data_input = reactive({
    req(input$file1)
    # convert UI to CSV
    x = read.csv(input$file1$datapath, stringsAsFactors=TRUE)
  ...
  Missing1 = reactive({
    # Require at least 1 column with missing data
    req(colnames(data()[apply(data(), 2, anyNA)]))
    # Create missing data object
    na.patterns = naclus(dplyr::select(data(), colnames(data())[apply(data(), 2, anyNA)]))
    # set plot size
    par(mar = c(5, 5, 5, 2))
    par(cex=0.9)
    # plot object
...
########################################
# Import data and examine missing data #
########################################
...
dev.off()', paste(strsplit(excPreds, ', ')), excPreds))
  })
  ...
  impute = reactive({
    req(input$radioImpute == 'Imputed')
    # select columns that contain NAs
    datax = data() %>%
      select_if(~ any(is.na(.)))
    
    # Loop through missing data df and replace with imputed data
    for(i in 1:length(names(datax))){
      datax[i] = eval(parse(text = sprintf('with(data(), Hmisc::impute(trans(), %s, data = data()))', colnames(datax[i]))))
    }
    datax
  })
  ...
  imputed_data = reactive({
    ...
    if (input$radioImpute == 'Imputed'){
      ...
      data_impute %>%
        ...
        dplyr::select(-input$selectImpute)
    } else
      data() 
  })
...
# Loop through missing data df and replace with imputed data
for(i in 1:length(names(datax))){
  datax[i] = eval(parse(text = sprintf("with(data, impute(trans, %%s, data = data))", colnames(datax[i]))))
}
...
# combine with imputed columns (previously calculated)
data_impute = cbind(data_impute, datax)

# reorder columns to match original dataframe
data_impute = data_impute %%>%%
  dplyr::select(names(data), -%s)
...
%s', input$survDuration_output, input$timeUnit_output, toString(survPrint_var())))
  ...
  output$lasso_imputePrint = renderPrint({
    x = c(input$eventElement_output)
    x = paste(shQuote(x), collapse=", ")
    lassopreds = toString(c(input$lassoPred))
...
Lassocoef %%>%%
  ...
  observeEvent(S_lasso(),{
    pred = lasso_impute() %>% dplyr::select(where(is.numeric))
    pred = names(pred) %>% discard(names(pred) %in% c(input$survDuration_output, input$eventCol_output))
    predCat = lasso_impute() %>% dplyr::select(where(is.factor))
  ...
  lassoFit = eventReactive(input$calcLasso, {
    ...
    set.seed(1)
    df = lasso_impute() %>% dplyr::select(matches(LassoPred_comp()))
  ...
  observeEvent(S_lasso(),{
    ...
    predCat = lasso_impute() %>% dplyr::select(where(is.factor))
    predCat = names(predCat) %>% discard(names(predCat) %in% c(input$survDuration_output, input$eventCol_output))
    updatePickerInput(session, "gLassoPred", choices= pred)
    updatePickerInput(session, "gLassoCat", choices= predCat)
  })
  
  # check for missing data in columns Variable
  grpLassoMissing = reactive ({
    req(imputed_data())
    # list all predictors
    lassPreds = imputed_data() %>%
      dplyr::select(input$gLassoPred, input$gLassoCat)
    
    #columns with missing data
    names(which(colSums(is.na(lassPreds))>0))
  })
  
  # Print columns with missing data
  output$grpLassoWarning = renderPrint({
    req(grpLassoMissing())
    sprintf('The following predictors have missing data: %s', paste(grpLassoMissing(), collapse = ", "))
  })
  
  # Group Lasso data set preparation
  gLassoPrep = eventReactive(input$action_glasso,{
    req(S())
    req(length(grpLassoMissing()) == 0) # ensure no missing data
    # list all predictors
    lassPreds = c(input$gLassoPred, input$gLassoCat)
    # list only categorical
    expand = c(input$gLassoCat)
    
    # Filter data sets
    data2 = imputed_data() %>%
      dplyr::select(all_of(lassPreds))
    
    expanddf = imputed_data() %>%
      dplyr::select(all_of(expand))
    
    # Create dummy data for categorical columns
    if(length(expand) != 0){
      for (i in 1:length(expand)){
        data2 = eval(parse(text=sprintf('data2 %%>%% mutate(dummy=1) %%>%% spread(key=%s,value=dummy, fill=0)', expand[i])))
        colName = c()
        for(j in levels(expanddf[,i])){
          newcolname = paste0(expand[i],'__',j)
          colName = append(colName, newcolname)
        }
        colnames2 = head(colnames(data2), -length(colName))
        colnames2 = append(colnames2, colName)
  ...
  gLasso = reactive({
    ...
    # specify groups
    group = colnames(gLassoPrep())
    group = str_remove(group, pattern = '__.*$')
    ...
    # Set random seed to allow repetitive results
    set.seed(1)
    
    # Create model
    cv.grpsurv(X, y, group, penalty="grLasso")
  })
  
  # Render plot
  output$gLassoPlot = renderPlot({
    req(gLasso())
    plot(gLasso())
  })
  
  # Value of log lambda at lowest cross validation value
  output$gLasso_minLambda = renderText({
    req(gLasso())
    log(gLasso()[["lambda"]][gLasso()[["min"]]])
  })
  
  # summary of group lasso fit
  ...
  # update lambda range
  observeEvent(gLasso(),{
    updateSliderInput(session, 'gLassolambda', max = round(log(gLasso()[["lambda"]][1]), digits = 2), min = round(log(gLasso()[["lambda"]][100]), digits = 2), value = log(gLasso()[["lambda"]][gLasso()[["min"]]]))
  })
  
  # print summary of group lasso
  output$gLassoSummary = renderPrint({
    # specify groups
    group = colnames(gLassoPrep())
    group = str_remove(group, pattern = '__.*$')
    group = factor(group, levels = unique(group))
    ...
    X = gLassoPrep()
    y = S()
    summary(cv.grpsurv(X, y, group, penalty="grLasso", lambda = exp(input$gLassolambda)))
  })
  
  # Show coefficient groups at lambda at given log lambda CV
  output$gLassoGroup = renderPrint({
    req(gLasso())
    req(input$gLassolambda)
    predict(gLasso(), type="groups", lambda = exp(input$gLassolambda))
  })
  
  # Print coefficients at given log lambda
  output$gLassoCoef = renderPrint({
    req(gLasso())
    req(input$gLassolambda)
    coef(gLasso(), lambda = exp(input$gLassolambda))
  })
  
  # Script for group Lasso
  output$RgLasso = renderPrint({
    cat(sprintf(
      '
###############
# Group Lasso #
...
# list all selected data
lassPreds = c(%s, %s)

# list only categorical
expand = c(%s)

# Remove any blank elements caused by no elements selected in either numeric or cat predictors
lassPreds = lassPreds[lassPreds != ""]
expand = expand[expand != ""]

# Filter data sets
gLassoData = imputed_data %%>%%
  dplyr::select(all_of(lassPreds))
...
if(length(expand) - 0){
  for (i in 1:length(expand)){
    gLassoData = eval(parse(text=sprintf("gLassoData %%%%>%%%% mutate(dummy=1) %%%%>%%%% spread(key=%%s,value=dummy, fill=0)", expand[i])))
    colName = c()
    for(j in levels(expanddf[,i])){
      newcolname = paste0(expand[i],"__",j)
      colName = append(colName, newcolname)
      }
    colnames2 = head(colnames(gLassoData), -length(colName))
    colnames2 = append(colnames2, colName)
    colnames(gLassoData) = colnames2
    }
  gLassoData
  } else {
  gLassoData
...
# perform group Lasso
cvfit = cv.grpsurv(gLassoData, S, group, penalty="grLasso")

# plot
plot(cvfit)

# Get summary of data at lowest cv
summary(cvfit)

# show gLasso selected predictors at lowest cross validation value
predict(cvfit, type="groups")

# show gLasso coefficients at lowest cross validation value
coef(cvfit)

# Set random seed to allow repetitive results
set.seed(1)

# adjust lambda on model
cvfit2 = cv.grpsurv(gLassoData, S, group, penalty="grLasso", lambda = exp(%s))

# Get new summary
summary(cvfit2)

# Get gLasso selected predictors at given lambda
predict(cvfit, type="groups", lambda = exp(%s))

# Get gLasso copefficients at given lambda
coef(cvfit, lambda = exp(%s))
',
      paste0("'",input$gLassoPred,"'", collapse = ', '), paste0("'",input$gLassoCat,"'", collapse = ', '),
      paste0("'",input$gLassoCat,"'", collapse = ', '),  input$gLassolambda,
      input$gLassolambda, input$gLassolambda))
  })
  ...
  somerPredList = reactive({
    ...
    for(i in names(imputed_data() %>% select_if(is.factor))){
  ...
  # Update select input for redundancy analysis
  observeEvent(imputed_data(), {
    updatePickerInput(session, 'redunCoxSelectPred', choices = names(imputed_data()))
  })
  
  # Create Redundancy analysis variable
  output$redunCox =  renderPrint({
    req(imputed_data)
    req(length(c(input$redunCoxSelectPred)) > 1)
    vars = paste(c(input$redunCoxSelectPred), collapse = ' + ')
    x = eval(parse(text = sprintf('with(imputed_data(), Hmisc::redun(~ %s, r2=input$redunCoxR2))', vars)))
    x$Out
  })
  ...
  observeEvent(S() , {
    pred = imputed_data() %>% dplyr::select(where(is.numeric))
    pred = names(pred) %>% discard(names(pred) %in% c(input$survDuration_output, input$eventCol_output))
    predCat = imputed_data() %>% dplyr::select(where(is.factor))
    predCat = names(predCat) %>% discard(names(predCat) %in% c(input$survDuration_output, input$eventCol_output))
  ...
  cox_initiate = eventReactive(input$cal_coxModel, {
    req(S())
    req(imputed_data())
    req(any(is.na(imputed_data() %>% dplyr::select(input$selectCat))) != TRUE)
    pred = c(input$selectPred)
    pCat = c(input$selectCat)
  ...
  observeEvent(input$cal_coxModel, {
    if (any(is.na(imputed_data() %>% dplyr::select(c(input$selectCat)))) == TRUE){
  ...
  # Give warning for number of predictors and 
  output$overPeramWarn = renderUI({
    req(cox())
    h3('Caution: Model performance can be comprimised when the number of covariates is close to the number of events. Lasso Regression can be used to reduce number of covariates although this can increase bias estimations', style = 'color:#DAA520')
  })
  
  output$overPeramVals = renderUI({
    req(cox())
    Events = as.vector(cox()$n[2])
    parameters = length(cox()$coefficients)
    h4(sprintf('This model comprises of %s covariates with a dataset of %s events ', parameters, Events), style = 'color:#191970')
  })
...
noise = (1-shrinkage)*100 # estimated percentage of fitted noise',
      ...
      somerPredList(), paste(c(input$redunCoxSelectPred), collapse = ' + '), input$redunCoxR2,
      fullCoxPrint_var(), input$survDuration_output, input$eventCol_output,
      x, fullCoxPrint_var()))
  })
...
plotINT(HR_rcs_delta, xlab = %s)',
  ...
  influence_var = reactive({
    req(length(cox()[["mmcolnames"]]) > 1)
    # Get residuals
    rr = resid(cox(), type = 'dfbeta')
    # modify colnames to remove any characters after '=' caused by categorical variables
  ...
  observeEvent(cox(), {
    req(influence_var())
    infPreds = colnames(influence_var())
    infPreds = str_replace_all(infPreds, pattern = "'", replacement = '*')
    updatePickerInput(session, 'selectInfluencePred', choices = infPreds)
  })
  
  # Create dynamic influence plot
  output$influence_plot = renderPlot({
    req(influence_var())
    
    # Get residuals and modify colnames to remove "'" caused by restricted cubic splines
    infPreds = colnames(influence_var())
    infPreds = str_replace_all(infPreds, pattern = "'", replacement = '*')
    # Get index number of residual which matches selected predictor
    indxResidual = which(infPreds == input$selectInfluencePred)
    # plot residuals
    eval(parse(text = sprintf('with(imputed_data(), plot(%s,influence_var()[,%s],xlab="%s",ylab="Influence"))', str_remove(input$selectInfluencePred, '\\*'), indxResidual, input$selectInfluencePred)))
  })
  
  # Identify overly influential observations - var
  overInfluenceDetect = reactive ({
    req(influence_var())
    # identify influence with cut off
    Infl = which.influence(cox(), input$inflCutoff)
    # Predictors - remove "'" caused by RCS
    preds = paste(colnames(influence_var()), collapse = ', ')
    preds = unique(str_replace_all(preds, "'", ''))
    # Create df with predictor values
    df = eval(parse(text = sprintf('with(imputed_data(), data.frame(%s))', preds)))
    #Show influence
    tibble::rownames_to_column(show.influence(Infl, df), 'Observation Id')
  })
  
  # DataTable showing overly influential observations
  output$overInfluenceTable = renderTable({
    req(nrow(overInfluenceDetect())>1)
    overInfluenceDetect()
  })
...
###############################
# Examine Predictor influence #
###############################

# Get residuals to calculate predictor influence
rr = resid(cox, type = "dfbeta")

# Remove any characters after "=" resulting from categorical dara
colnames(rr) = str_remove(colnames(rr), pattern = "=.*$")
...
# See residual col names
colnames(rr)

# Plot influence 
plot(%s,rr[["%s"]],xlab="%s",ylab="influence")

# Dataframe of overly Influential observations
Infl = which.influence(cox, %s)
df = with(imputed_data, data.frame(%s))
#Show influence
tibble::rownames_to_column(show.influence(Infl, df), "Observation Id")',
      input$selectPhPlot, input$selectPhPlot, str_remove(input$selectInfluencePred, '\\*'),
      str_replace(input$selectInfluencePred, '\\*', '.'), str_remove(input$selectInfluencePred, '\\*'),
      input$inflCutoff, preds))
  })
  ...
  val = eventReactive(input$action_validate, {
    ...
    withProgress( message = 'Validating', {
      ...
      val = rms::validate(cox, u = input$Validate_timeInc, B = input$Validate_bootstrap)
      ...
      vals=c(cox[["time.inc"]], c.index, c.index*100, (1-shrinkage)*100)
      vals
    })
  })
  ...
  output$validate = renderText({
    req(val())
    # text describing validation
    sprintf('At time %s, Concordance index equals %f. Meaning that the model correctly ranks %f%% of the predicted outcome to observed data. Ideally this would want to be > 0.8. Furthernore the model is fitting %f%% noise. This should ideally be < 10%%', val()[1], val()[2], val()[3], val()[4])
  })
...
plot(cal, subtitles = FALSE)
  ...
  output$allInputs = renderUI({
    ...
    preds = c(input$selectPred, input$selectOrdCat, input$selectLog, input$selectCluster, 
              input$selectStrat, input$selectRCS3, input$selectRCS4, input$selectRCS5)
    predsCat = input$selectCat
  ...
  # Get Predictor names and inputted values for survival estimate
...
1 - Cidx[1]', paste(strsplit(coxPredictors, ', ')), input$extSurvDuration_output,
input$extSurvDuration_output, input$extEventCol_output,x, toString(predCoxExt()),
input$ols_selector, input$extSurvDuration_output, input$extSurvDuration_output,
input$extEventCol_output, input$extEventCol_output))
  })
  
  # Print R Script for external validation
  output$RextCal = renderPrint({
