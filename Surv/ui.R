    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel('Lasso Regression',
                 h3('Lasso regression can be used to assist in selecting continious predictors when developing Cox Model. If you also have categorical data, you may want to use "Group Lasso" predictor selection'),
                 h5(em('This uses the "glmnet" package. (T Hastie et al., 2014)'), style= "color:#800000"), 
        ...
        tabPanel('Group Lasso Regression',
                 h3('Group Lasso regression can be used to assist in selecting categorical AND continious predictors when developing Cox Model. If you only have continious data, you may want to use "Lasso" predictor selection instead'),
                 h5(em('This uses the "grpreg" package. (P Breheny et al., 2021)'), style= "color:#800000"), 
                 h5('Note that rows with event time 0 or lower have automatically been removed from lasso regression', style = "color:red"),
                 h3('Select Predictors'),
                 fluidRow(column(4,
                                 pickerInput('gLassoPred', 'Select Numeric Predictors', choices = c('No data entered'), options = list(`actions-box` = TRUE), multiple = TRUE),
                                 pickerInput('gLassoCat', 'Select Categorical Predictors', choices = c('No data entered'), options = list(`actions-box` = TRUE), multiple = TRUE)),
                          column(8,
                                 h4('Caution: Group Lasso will not work function if there is missing data.', style="color:darkred"),
                                 strong(em(textOutput('grpLassoWarning'), style="font-size:100%;color:#6E2C00")),
                                 h4('Caution: Using Lasso regression can cause innaccurate confidence intervals in later models', style="color:red"))),
                 actionButton('action_glasso', 'Group Lasso', class = "btn btn-success"),
                 plotOutput('gLassoPlot'),
                 h4('Log Lambda at lowest cross validation is:'),
                 h4(em(textOutput('gLasso_minLambda')), style = "color:#800000"),
                 h4('Summary of Model'),
                 verbatimTextOutput('summaryfit'),
                 hr(),
                 h3('Summary of Model at given log lambda - Default at lowest cross validation score'),
                 sliderInput('gLassolambda', 'log Lambda value:', min = 0, max = 1, value = 0.5),
                 h4('Model Summary'),
                 verbatimTextOutput('gLassoSummary'),
                 h4('Predictors at given lambda - default value at lowest cross validation score'),
                 verbatimTextOutput('gLassoGroup'),
                 h4('predictor coefficients'),
                 verbatimTextOutput('gLassoCoef'),
                 h3('R Script'),
                 verbatimTextOutput('RgLasso')),
        ...
        tabPanel('Summarise Results',
                 ...
                 h5('Caution: This does not accomodate transformed predictors (ie log transformation)  and this would need to be done manually', style = "color:red"),
                 uiOutput("allInputs"),
